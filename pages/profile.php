...
if(!empty($_POST['tfa_code'])){
    if($tfa->verifyCode($_SESSION['tfa_secret'], $_POST['tfa_code'])){
        $q = $db->prepare('UPDATE user SET secret = :secret WHERE id = :id');
        $q->bindValue('secret', $_SESSION['tfa_secret']);
        $q->bindValue('id', $_SESSION['user_id']);
        $q->execute();
        header('location:/autobonplan/pages/logout.php');
    }else{
        header('location:/autobonplan/pages/index.php');
    }
}

$userReq = $db->prepare('SELECT * FROM user WHERE id = :id');
...
if(!$user['secret']){
    ...
    <div id="QRCode-container">
        <a id="logout-btn" href="/autobonplan/pages/logout.php"><img src="../images/cancel.png" alt="cross"></a>
